...
public class targetset {
    ...
    //Find substring pattern in given string 
    public static int isSSmatch(String a, String b, int n, int m , int[][] dp){
        if(n==0 || m==0){
            if(m==0) return dp[n][m] = 1;
            return dp[n][m] = 0;
        }
        if(dp[n][m] != -1) return dp[n][m];
        char ch1 = a.charAt(n-1);
        char ch2 = b.charAt(m-1);
        if(ch1 == ch2 || ch2 == '*'){
            boolean res= false;
            res = res || isSSmatch(a, b, n-1, m-1, dp) == 1;
            res = res || isSSmatch(a, b, n-1, m, dp) == 1;

            return dp[n][m] = res ? 1: 0;

        }else{
            return dp[n][m] = isSSmatch(a, b, n-1, b.length(), dp);
        }
    }
    ...
    public static void subStringMatch(){
        ...
        boolean res = isSSmatch(a,b,n,m,dp) == 1;
